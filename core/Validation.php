...
class Validation
{
				...
				if(in_array($key, $this->rules['fields'])){
					...
					if(array_key_exists('not_empty', $this->rules) && in_array($key, $this->rules['not_empty'])){
						...
						if($value == '')
							...
							elseif(array_key_exists('not_url', $this->rules) && in_array($key, $this->rules['not_url']) && !preg_match("/^[a-zA-Z]/", $value)){
	...
	private function unique($query, $pk = null){
		...
		if($pk == null)
			$result = SQL::Instance()->Select("SELECT * FROM $this->table WHERE $query");
		else{
			$result = SQL::Instance()->Select("SELECT * FROM $this->table WHERE ($query) AND $primary_key<>$pk");
		}	
